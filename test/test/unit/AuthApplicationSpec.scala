...
class AuthApplicationSpec extends PlaySpec with OneServerPerSuite{
  ...
  "Application" should {
    ...
    "Allow Contributors to create news posts" in {
       setup
       val json = Json.parse("""{"id":"f26c0ddc-214e-4d3a-a8c8-0deec3045e75","title":"title","postType":1,"dateCreated":"2012-04-21","author":"Andrew","content":"12345678","extraData":"{ \"thumb\":\"assets/leek.jpg\"}"}""")
    ...
    "Not Allow contributors to create biography posts" in {
      setup
      val json = Json.parse("""{"id":"f26c0ddc-214e-4d3a-a8c8-0deec3045e75","title":"title","postType":4,"dateCreated":"2012-04-21","author":"Andrew","content":"12345678","extraData":"{ \"thumb\":\"assets/leek.jpg\"}"}""")
      val result = route(FakeRequest(POST, controllers.routes.Authorised.submitBlog().url,FakeHeaders(),json).withLoggedIn(config)("Contributor")).get
